    ...
    public class Entry extends EntryListWidget.Entry<WildfireBreastPresetList.Entry> {
        ...
        private Entry(final BreastPresetListEntry nInfo) {
            ...
            btnOpenGUI = new WildfireButton(0, 0, getRowWidth() - 6, itemHeight, Text.empty(), button -> {
                ...
                var cfg = plr.config();
                cfg.bustSize.copy(nInfo.data.bustSize);
                cfg.breastsXOffset.copy(nInfo.data.breastsXOffset);
                cfg.breastsYOffset.copy(nInfo.data.breastsYOffset);
                cfg.breastsZOffset.copy(nInfo.data.breastsZOffset);
                cfg.breastsCleavage.copy(nInfo.data.cleavage);
                cfg.breastsUniboob.copy(nInfo.data.uniboob);
                plr.save();
            });
        }
